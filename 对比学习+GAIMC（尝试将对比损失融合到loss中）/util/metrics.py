...
def contingency_matrix(labels_true, labels_pred, eps=None, sparse=False):
    ...
    labels_true : int array, shape = [n_samples]
        Ground truth class labels to be used as a reference(地面真实类标签用作参考)
    ...
    eps : None or float, optional.
        ...
        (如果是浮点数，则将该值添加到偶然性中的所有值

        矩阵。这有助于阻止NaN传播。

        如果' ' None ' '，则不进行任何调整。)

    sparse(稀疏的） : boolean, optional.
        If True, return a sparse CSR continency matrix. If ``eps is not None``,
        and ``sparse is True``, will throw ValueError.
        （布尔,可选的。
        如果为True，返回一个稀疏CSR连续性矩阵。如果' ' eps不是None '，

而' ' sparse is True ' '则会抛出ValueError。）
        .. versionadded:: 0.18
...
# from sklearn.utils.fixes import comb # (原)
